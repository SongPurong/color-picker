<template>
  <div class="compact">
    <div class="compact-row" v-for="(row, index) in palette" :key="index">
      <div
        class="compact-row-item"
        v-for="color in row"
        :key="color"
...
export default {
  ...
  methods: {
    emitChange(color) {
      this.$emit("cChange", color);
    },
    computedBgStyle(color) {
